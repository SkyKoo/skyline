...
class File : noncopyable
{
 public:
  ...
  {
    char buf[n];
    // read file with 1 byte
    // n is the number of read bytes
    // if nr != n, means there is a error or at end
  ...
  int64_t readInt64()
  {
    ...
    return be64toh(x); // convert the byte order
  }
  ...
  int32_t readInt32()
  {
    ...
    return be32toh(x); // convert the byte order
  }
...
// RFC 8536: https://www.ftc-editor.org/rfc/rfc8536.html
...
const TimeZone::Data::LocalTime* TimeZone::Data::findLocalTime(int64_t utcTime) const
{
  ...
  {
    // FIXME: should be first non dst time zoon
    local = &localtimes.front();
  }
  else
  {
    // use upper_bound should make sure the transitions is sorted, so where to do the sorting?
    ...
    const struct DateTime& lt, bool postTransition) const
{
  ...
  //  3  1990-04-15 17:00:00Z   0      8.0   1990-09-16 01:00:00   1990-09-16 01:59:59
  //  4  1991-04-13 18:00:00Z   1      9.0   1991-04-14 03:00:00   1991-04-14 01:59:59
  //  5  1991-09-14 17:00:00Z   0      8.0   1991-04-14 01:00:00
